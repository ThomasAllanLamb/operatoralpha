...
(function () {
  ...
  d3.csv(
    "api/bounded"
    , function(row) {
      //each cell is in string format, so convert to numbers. Javascript converts numbers which are too high or too low are converted to infinity. Discard those rows because D3 can't plot them.
      var formattedRow = {};
      //var columns = ["m","n","u","r"];
      var columns = ["m","n","u","r_lower","r_upper"]
      for (var i = 0; i <= columns.length-1; i++)
      {
        var column = columns[i];
        var formattedValue = parseFloat(row[column]);

        //parseFloat can return unplottable values like infinity and NaN. Exclude those
        //???: we currently take undefined properties of points to be meaningful. But this will allow for confusion with bad. Can we refactor so that this is not a potential problem?
        if (!(formattedValue >= -1*Number.MAX_VALUE && formattedValue <= Number.MAX_VALUE))
        {
          console.warn("Unplottable property",column,row);
          //...discard this row
          return undefined;
        }
    ...
    , function(error, data) {
      ...
      var line = d3.line()
        ...
        .y(function(d) { return yScale(d.r_lower); })
      ;
